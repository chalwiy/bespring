#!/usr/bin/env python3
"""Audit requested pt-BR product dossiers, SEO data and directory links."""
...
import build_portuguese_product_pages as build
...
def main() -> None:
    ...
    for rel in rels:
        page = ROOT / "pt" / rel
        if not page.exists():
            issues.append(f"missing page: pt/{rel}")
            continue
        raw = page.read_text(encoding="utf-8")
        doc = html.fromstring(raw)
        if doc.get("lang") != "pt-BR":
            issues.append(f"wrong html lang: pt/{rel}")
        expected = f"{BASE}/pt/{rel}"
        if doc.xpath("string(//link[@rel='canonical']/@href)") != expected:
            issues.append(f"wrong canonical: pt/{rel}")
        alternates = {
        ...
        for node in doc.xpath("//*[@href or @src]"):
            ...
            if target is not None and not target.exists():
                issues.append(f"broken local link: pt/{rel} -> {value}")
        if expected not in sitemap_urls:
            issues.append(f"missing sitemap URL: pt/{rel}")
        for phrase in (
            ...
            if phrase in raw:
                issues.append(f"untranslated boilerplate {phrase!r}: pt/{rel}")
    ...
    for category in build.CATEGORIES:
        ...
        target = html.parse(
            str(ROOT / "pt" / "products" / f"{category}.html"),
            parser=html.HTMLParser(encoding="utf-8"),
        )
        expected_links = {
    ...
    print(f"Audited {len(rels)} requested Portuguese pages: {dict(counts)}")
