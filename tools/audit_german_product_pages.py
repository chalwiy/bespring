#!/usr/bin/env python3
"""Audit requested German product dossiers, SEO data and directory links."""
...
import build_german_product_pages as build
...
def main() -> None:
    ...
    for rel in rels:
        page = ROOT / "de" / rel
        if not page.exists():
            issues.append(f"missing page: de/{rel}")
            continue
        raw = page.read_text(encoding="utf-8")
        doc = html.fromstring(raw)
        if doc.get("lang") != "de":
            issues.append(f"wrong html lang: de/{rel}")
        expected = f"{BASE}/de/{rel}"
        if doc.xpath("string(//link[@rel='canonical']/@href)") != expected:
            issues.append(f"wrong canonical: de/{rel}")
        alternates = {
        ...
        for node in doc.xpath("//*[@href or @src]"):
            ...
            if target is not None and not target.exists():
                issues.append(f"broken local link: de/{rel} -> {value}")
        if expected not in sitemap_urls:
            issues.append(f"missing sitemap URL: de/{rel}")
        for phrase in (
            ...
            if phrase in raw:
                issues.append(f"untranslated boilerplate {phrase!r}: de/{rel}")
    ...
    for category in build.CATEGORIES:
        ...
        target = html.parse(
            str(ROOT / "de" / "products" / f"{category}.html"),
            parser=html.HTMLParser(encoding="utf-8"),
        )
        expected_links = {
    ...
    print(f"Audited {len(rels)} requested German pages: {dict(counts)}")
