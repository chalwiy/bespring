#!/usr/bin/env python3
"""Audit requested Arabic product dossiers, RTL, SEO and directory links."""
...
import build_arabic_product_pages as build
...
def main() -> None:
    ...
    for rel in rels:
        page = ROOT / "ar" / rel
        if not page.exists():
            issues.append(f"missing page: ar/{rel}")
            continue
        raw = page.read_text(encoding="utf-8")
        doc = html.fromstring(raw)
        if doc.get("lang") != "ar" or doc.get("dir") != "rtl":
            issues.append(f"wrong Arabic lang/dir: ar/{rel}")
        expected = f"{BASE}/ar/{rel}"
        if doc.xpath("string(//link[@rel='canonical']/@href)") != expected:
            issues.append(f"wrong canonical: ar/{rel}")
        alternates = {
        ...
        if any(alternates.get(key) != value for key, value in required.items()):
            issues.append(f"hreflang mismatch: ar/{rel}")
        ...
        if not re.search(r"[\u0600-\u06ff]", title):
            issues.append(f"title lacks Arabic text: ar/{rel}")
        if not 25 <= len(title) <= 90:
            warnings.append(f"title length {len(title)}: ar/{rel}")
        if not 90 <= len(description) <= 180:
            warnings.append(f"description length {len(description)}: ar/{rel}")
        if "�" in raw or re.search(r"(?:Ã.|Â.|â€|ðŸ)", raw):
            issues.append(f"encoding artifact: ar/{rel}")
        for script in doc.xpath("//script[@type='application/ld+json']"):
            try:
                json.loads(script.text or "")
            except json.JSONDecodeError as exc:
                issues.append(f"invalid JSON-LD: ar/{rel}: {exc}")
        for node in doc.xpath("//*[@href or @src]"):
            ...
            if target is not None and not target.exists():
                issues.append(f"broken local link: ar/{rel} -> {value}")
        if expected not in sitemap_urls:
            issues.append(f"missing sitemap URL: ar/{rel}")
        for phrase in (
            ...
            if phrase in raw:
                issues.append(f"untranslated boilerplate {phrase!r}: ar/{rel}")
    ...
    for category in build.CATEGORIES:
        ...
        target = html.parse(
            str(ROOT / "ar" / "products" / f"{category}.html"),
            parser=html.HTMLParser(encoding="utf-8"),
        )
        expected_links = {
    ...
    print(f"Audited {len(rels)} requested Arabic pages: {dict(counts)}")
